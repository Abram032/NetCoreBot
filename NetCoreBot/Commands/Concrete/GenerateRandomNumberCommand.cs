...
namespace NetCoreBot.Commands.Concrete
{
    class GenerateRandomNumberCommand : ICommand
    {
        ...
        public void Execute()
        {
            ICommandReciever _reciever = new CommandReciever(_messageDetails);
            int? rng = null;
            string error = "Invalid use of command!\n";
            if(AreValid(_parameters) == false);
            {
                _reciever.ReturnStatus(error);
                //return;
            }
            ...
            if(rng == null)
                ...
                _reciever.ReturnStatus(rng.ToString());
            return;
        }
